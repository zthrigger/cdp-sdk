...
@JsonDeserialize(using = X402McpRequestId.X402McpRequestIdDeserializer.class)
@JsonSerialize(using = X402McpRequestId.X402McpRequestIdSerializer.class)
public class X402McpRequestId extends AbstractOpenApiSchema {
    private static final Logger log = Logger.getLogger(X402McpRequestId.class.getName());

    public static class X402McpRequestIdSerializer extends StdSerializer<X402McpRequestId> {
        public X402McpRequestIdSerializer(Class<X402McpRequestId> t) {
            super(t);
        }

        public X402McpRequestIdSerializer() {
            this(null);
        }

        @Override
        public void serialize(X402McpRequestId value, JsonGenerator jgen, SerializerProvider provider) throws IOException, JsonProcessingException {
            jgen.writeObject(value.getActualInstance());
        }
    }

    public static class X402McpRequestIdDeserializer extends StdDeserializer<X402McpRequestId> {
        public X402McpRequestIdDeserializer() {
            this(X402McpRequestId.class);
        }

        public X402McpRequestIdDeserializer(Class<?> vc) {
            super(vc);
        }

        @Override
        public X402McpRequestId deserialize(JsonParser jp, DeserializationContext ctxt) throws IOException, JsonProcessingException {
            ...
            if (match == 1) {
                X402McpRequestId ret = new X402McpRequestId();
                ret.setActualInstance(deserialized);
                return ret;
            }
            throw new IOException(String.format("Failed deserialization for X402McpRequestId: %d classes match result, expected 1", match));
        }
         ...
         */
        @Override
        public X402McpRequestId getNullValue(DeserializationContext ctxt) throws JsonMappingException {
            throw new JsonMappingException(ctxt.getParser(), "X402McpRequestId cannot be null");
        }
    }
    ...
    public X402McpRequestId() {
        super("oneOf", Boolean.FALSE);
    }

    public X402McpRequestId(Integer o) {
        super("oneOf", Boolean.FALSE);
        setActualInstance(o);
    }

    public X402McpRequestId(String o) {
        super("oneOf", Boolean.FALSE);
        setActualInstance(o);
    }
    ...
    static {
        ...
        JSON.registerDescendants(X402McpRequestId.class, Collections.unmodifiableMap(schemas));
    }

    @Override
    public Map<String, Class<?>> getSchemas() {
        return X402McpRequestId.schemas;
    }
