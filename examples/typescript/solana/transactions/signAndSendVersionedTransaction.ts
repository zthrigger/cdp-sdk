 ...
 * 3. Signs a versioned (v0) transaction with CDP to send SOL to a destination address
...
async function main(sourceAddress?: string) {
  ...
  try {
    ...
    // version: 0 creates a versioned (v0) transaction message
    const txMsg = pipe(
